  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (result.success) {
        ...
        setError(result.error || 'There was an error submitting your feedback.');
      }
    } catch (err) {
  ...
  if (success) {
    return (
      <div className="text-center space-y-4 py-8 animate-in fade-in zoom-in duration-500">
        ...
        <p className="text-muted-foreground">Your feedback has been successfully submitted and helps us improve our sessions.</p>
      </div>
    );
  }
    ...
    <div className="space-y-8">
      ...
      <div className="min-h-[250px] flex flex-col justify-center animate-in slide-in-from-right-4 fade-in duration-300">
        ...
        {step === 1 && (
          <div className="space-y-4">
            <Label htmlFor="feelingInArms" className="text-xl md:text-2xl font-semibold leading-relaxed block">
              How did you feel in the arms of your facilitator? <span className="text-destructive">*</span>
            </Label>
            <Textarea
              id="feelingInArms"
              value={feelingInArms}
        ...
        {step === 2 && (
          <div className="space-y-4">
            <Label htmlFor="overallExperience" className="text-xl md:text-2xl font-semibold leading-relaxed block">
              Overall experience of your Janzu session <span className="text-destructive">*</span>
            </Label>
            <Textarea
        ...
        {step === 3 && (
          <div className="space-y-6">
            <Label className="text-xl md:text-2xl font-semibold leading-relaxed block">
              Did you feel supported at the end of the session? <span className="text-destructive">*</span>
            </Label>
            <div className="space-y-3">
              {['Yes', 'Not enough', 'Other'].map((option) => (
                <div 
                  key={option} 
                  className={`flex items-center p-4 border rounded-lg cursor-pointer transition-colors ${feltSupported === option ? 'border-primary bg-primary/5' : 'hover:bg-muted/50'}`}
                  onClick={() => setFeltSupported(option)}
                >
                  <input
                    type="radio"
                    name="feltSupported"
                    value={option}
                    checked={feltSupported === option}
                    onChange={() => setFeltSupported(option)}
        ...
        {step === 4 && (
          <div className="space-y-4">
            <Label htmlFor="additionalComments" className="text-xl md:text-2xl font-semibold leading-relaxed block">
              Is there anything else about your session you'd like to comment on? <span className="text-destructive">*</span>
            </Label>
            <Textarea
        ...
        {step === 5 && (
          <div className="space-y-6">
            <Label className="text-xl md:text-2xl font-semibold leading-relaxed block">
              Would you like to continue your process in the water? <span className="text-destructive">*</span>
            </Label>
            <div className="space-y-3">
              {['I would like to receive another session', 'No, thank you'].map((option) => (
                <div 
                  key={option} 
                  className={`flex items-center p-4 border rounded-lg cursor-pointer transition-colors ${continueProcess === option ? 'border-primary bg-primary/5' : 'hover:bg-muted/50'}`}
                  onClick={() => setContinueProcess(option)}
                >
                  <input
                    type="radio"
                    name="continueProcess"
                    value={option}
                    checked={continueProcess === option}
                    onChange={() => setContinueProcess(option)}
                    className="h-4 w-4 text-primary focus:ring-primary border-gray-300"
                  />
        ...
        {step === 6 && (
          <div className="space-y-6 text-center">
            ...
            <p className="text-muted-foreground text-lg mb-8">One final question before you submit your feedback.</p>
            ...
            >
              ...
              <Label htmlFor="interestedInLearning" className="ml-4 text-xl font-medium cursor-pointer select-none">
                I would be interested in learning Janzu
              </Label>
            </div>
          </div>
        )}
      ...
      <div className="flex items-center justify-between pt-8 border-t">
        ...
        {step < totalSteps ? (
          ...
          <Button 
            ...
            className="text-base h-11 px-8"
          >
            {loading ? 'Submitting...' : 'Submit Feedback'} <Send className="ml-2 h-4 w-4" />
          </Button>
        )}
      </div>
    </div>
